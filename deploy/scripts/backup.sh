...
cleanup() {
  if [[ $SERVER_STOPPED -eq 1 ]]; then
    compose up --detach --wait --wait-timeout 120 server caddy >/dev/null
  fi
...
compose stop caddy server >/dev/null
SERVER_STOPPED=1
compose exec -T mysql sh -eu -c 'MYSQL_PWD="$MYSQL_PASSWORD" exec mysqldump --single-transaction --routines --events --hex-blob --set-gtid-purged=OFF --default-character-set=utf8mb4 --user="$MYSQL_USER" "$MYSQL_DATABASE"' \
  | gzip --best > "$WORK_DIRECTORY/mysql.sql.gz"
compose run --rm --no-deps --entrypoint tar server --numeric-owner -cf - -C /var/lib/remote-control-hub . \
...
for FILE_NAME in metadata.env mysql.sql.gz.age server-state.tar.gz.age; do
